...
- **The build moves from tsup to tsdown.** tsup 8.5.1, its last release, forces `baseUrl` into its declaration build, which TypeScript 6 fails as deprecated (TS5101), and that build calls the classic compiler API, which `typescript@7` does not ship. tsdown emits the same package: `npm pack` lists the same 38 files, `dist/index.d.ts` exports the same 199 symbols with the same types, and each entry is still one self-contained bundle. Under TypeScript 7 its declaration plugin switches to the native compiler, so the next TypeScript major needs no build change. `zod`, which `src/mcp/server.ts` imports but only `@modelcontextprotocol/sdk` depends on, is still bundled into `dist/mcp/server.js`; it is now a declared exception, and any other package from `node_modules` that lands in a bundle fails the build. `test/build-output.test.ts` pins the output shape. Building from source needs Node 22.18+, tsdown's floor; the published package still runs on Node 20+, and CI tests it on Node 20 and 22 against a build made on Node 22.
...
- **The build runs again under TypeScript 6.** Every build after the #8 dependency bump stopped at the declaration step with TS5101, although `tsconfig.json` sets no `baseUrl`: tsup injected it. #31 silenced the deprecation for tsup's pass as a stopgap; the move to tsdown (above) removes the cause, and the silence with it. CI's path filters now list every file its steps read (`tsdown.config.ts`, `scripts/postbuild.js`, `vitest.config.ts`, `eslint.config.js`); before #31, a change to the build config alone could not trigger the required checks.
