...
- **Handbook: publish-state ≠ registry publish.** Docs now state that `src/sync/publish.ts` GETs npm / PyPI / GitHub Releases and upserts `repo_published_versions` only — the filename is inventory, not a mutator. MCP `repo_versions` is a DB-only read; CLI `rk versions --refresh` may GET registries. No publish APIs added.
