...
- **Product docs no longer prescribe studio machine paths.** `KNOWLEDGE-CONTRACT.md`, `THE-CLAUDE-GAMES.md`, `REMEDIATION-INSTRUCTIONS.md`, and `ROADMAP.md` now use repo-relative filenames (`AUDIT-CONTRACT.md`, `data/knowledge.db`) or `<workspace>` / `<repo-root>` placeholders. ROADMAP multi-rig path examples are labeled operator narrative; companion Claude memory files are operator-only. (STUDY-RK-029)
