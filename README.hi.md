...
| `rk note <slug>` | `--type` और `--content` के साथ एक लिखित नोट जोड़ें (थीसिस, आर्किटेक्चर, चेतावनी आदि); `--delete` किसी नोट को `--type` + `--title` का उपयोग करके हटा देता है। |
...
### वर्गीकरण (संस्करण 2.1.1)

| कमांड | विवरण |
|---------|-------------|
| `rk classify <slug> [--status <s>] [--stage <s>] [--category <c>]` | चयनित जीवनचक्र फ़ील्ड सेट करें — `status` (`सक्रिय`/`रुका हुआ`/`संग्रहित`/`अज्ञात`), मुक्त-स्वरूप `stage` (जैसे `भेजा गया`, `चरण 1`), और `category` (`उत्पाद`/`उपकरण`/`लाइब्रेरी`/`प्रयोग`/`ब्लूप्रिंट`/`विपणन`)। इन्हें `sync` या `scan` द्वारा नहीं भरा जाता है; `stage`/`category` को साफ़ करने के लिए `""` पास करें। |
