...
1. **Node.js 20 or later** -- check with `node --version`. This package's `engines.node` is `>=20`. The locked `better-sqlite3` addon's own `engines` list is vendor majors, not a prebuild-coverage claim.
